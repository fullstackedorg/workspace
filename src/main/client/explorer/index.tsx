...
function Explorers(props: { explorerRef: RefObject<Explorer> }) {
    ...
    return <div className={"explorer-with-tabs"}>
        ...
        <div className={"tabs"}>
            ...
            {endpoints?.map((({ name, endpoint, helloResponse }, index) => {
                ...
                >
                    ...
                    {showRename && <Rename name={name} didSubmit={async newName => {
                        await client.put().updateStorageName(endpoint, newName);
        ...
        <div>
            {
                currentTab === null
                    ...
                    : typeof endpoints.at(currentTab).helloResponse === "boolean"
                        ...
                        ? <Explorer
                            options={{ showDeleteButtons, showHiddenFiles }}
                            origin={endpoints.at(currentTab).endpoint}
                            action={item => item.isDir && <Pull itemKey={item.key} storage={endpoints.at(currentTab)} />}
                        />
                        : <pre>{JSON.stringify(endpoints.at(currentTab).helloResponse, null, 2)}</pre>
            }
...
function Push(props: { 
    ...
    if(availableEndpoint.length === 0)
        ...
        return <button 
            ...
            onClick={(e) => {
                e.stopPropagation();
                client.post().push(props.itemKey, availableEndpoint.at(0).endpoint);
            }}
        >
            Sync
        </button>
...
function Pull(props: {
    ...
    return <button 
        ...
        onClick={(e) => {
            e.stopPropagation();
            client.post().pull(props.storage.endpoint, props.itemKey);
        }}
    >
        Sync
    </button>
...
function Rename(props: {name: string, didSubmit: (value: string) => void}){
    const [value, setValue] = useState(props.name);

    return <div className="rename">
        <form onSubmit={e => {
            e.preventDefault();
            props.didSubmit(value);
        }}>
